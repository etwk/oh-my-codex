...
| `pre-tool-use` | `PreToolUse` (`Bash`) | `pre-tool-use` | native-partial | Current native scope is Bash-only; built-in native behavior cautions on `rm -rf dist`, blocks inspectable inline `git commit` commands until Lore-format structure is present only when explicitly opted in with `OMX_LORE_COMMIT_GUARD=1`, and emits non-blocking document-refresh warnings for mapped staged commit changes that lack rule-scoped docs/spec refresh evidence |
...
Lore-format commit enforcement requires at least one Lore trailer. Optional conventional footers such as `Signed-off-by:`, `Reviewed-by:`, `Acked-by:`, and `Co-authored-by:` may follow Lore trailers, but they do not satisfy the Lore trailer requirement by themselves.
...
commit messages while keeping OMX-managed native hooks installed, set
`OMX_LORE_COMMIT_GUARD` to `1`, `true`, `yes`, or `on`.
